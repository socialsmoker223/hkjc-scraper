...
- [Using the Analytics Module](#using-the-analytics-module)
...
The analytics module provides statistical analysis functions for horse racing data. All functions accept lists of records loaded from JSON files.
...
with open("data/performance_2026-03-01.json") as f:
...
stats = calculate_jockey_performance(performances, recent_races=10)

# Example output for a jockey
{
    "BAC": {
        "name": "Moreira",
        "total_rides": 143,
        ...
        "place_rate": 0.342,
        "top4_rate": 0.447,
        "recent_form": ["1", "3", "2", "4", "DISQ", "1", "2", "3", "5", "1"],
        "avg_finish": 3.42,
        "by_track": {
            "ST": {"wins": 18, "rides": 89, "win_rate": 0.202},
            "HV": {"wins": 10, "rides": 54, "win_rate": 0.185}
        },
        "best_draws": [7, 5, 3],
        "avg_odds": 8.5
    }
...
stats = calculate_trainer_performance(performances)
```
...
# Analyze draw advantage by track and distance
bias = calculate_draw_bias(performances, races)
...
# Example output
{
    "ST-1200": {
        ...
        "total_races": 45,
        "win_stats": [
            {"draw": 1, "wins": 8, "runs": 45, "win_rate": 0.178},
            {"draw": 2, "wins": 6, "runs": 45, "win_rate": 0.133},
            ...
        ]
    }
}
...
# Analyze recent horse form
form = calculate_horse_form(performances, recent_races=6)
...
summary = generate_racing_summary(performances, races)
...
# Example output
{
    ...
    "total_runners": 143,
    "top_jockeys": [
        {"jockey_id": "BAC", "name": "Moreira", "wins": 3, "win_rate": 0.273}
    ],
    "top_trainers": [
        {"trainer_id": "MY", "name": "Mo", "wins": 2, "win_rate": 0.182}
    ],
    "track_biases": {...},
    "draw_biases": {...}
}
