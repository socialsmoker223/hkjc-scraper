...
def main():
    ...
    group.add_argument(
        ...
        nargs=2, 
        metavar=('START', 'END'),
        help="Scrape a range of dates (YYYY/MM/DD)"
    )
    group.add_argument(
        "--backfill",
        nargs=2,
        metavar=('START', 'END'),
        help="Backfill mode: scrape range of dates (YYYY/MM/DD)"
    )
    group.add_argument(
        "--update",
        action="store_true",
        help="Update mode: scrape from last DB entry to today"
    )

    # Optional flags
    parser.add_argument(
        "--start", 
        help="Start date for range scraping (deprecated, use --date-range instead)"
    )
    parser.add_argument(
        "--end", 
        help="End date for range scraping"
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
